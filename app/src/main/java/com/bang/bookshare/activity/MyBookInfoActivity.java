...
public class MyBookInfoActivity extends FrameActivity {
    ...
    private void initVariable() {
        ...
        borrowed = ecListEntity.getIsBorrowed();
        etBookName.setText(ecListEntity.getBookName());
        etAuthor.setText(ecListEntity.getBookAuthor());
        etBookCollections.setText(ecListEntity.getBookCollections());
        etBookProfile.setText(ecListEntity.getBookProfile());
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_save:
                ...
                if (bookName.isEmpty()) {
                    ...
                    // 请求网络更新服务器与本地图书信息
                    updateBook(String.valueOf(ecListEntity.getId()), bookName, bookAuthor, borrowed, bookCollections, bookProfile, lastUpdateTime);
                }
                break;
     ...
     * @param bookProfile
     */
    private void updateBook(final String id, final String bookName, final String bookAuthor, final String borrowed, final String bookCollections, final String bookProfile, final String lastUpdateTime) {
        //先new出一个监听器，设置好监听
...
//                        params.put("publisherName", publisherName);
                        ...
                        params.put("userId", PreferencesUtils.getUserId(MyBookInfoActivity.this));
                        params.put("userName", PreferencesUtils.getUserName(MyBookInfoActivity.this));
                        params.put("bookCode", getString(R.string.book) + id);
                        params.put("lastUpdateTime", lastUpdateTime);
                        params.put("lastUpdater", PreferencesUtils.getUserName(MyBookInfoActivity.this));
                        VolleyHttpRequest.String_request(HttpPathUtil.updateBook(), params, volleyRequest);
                        break;
